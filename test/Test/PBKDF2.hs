  ...
  , testGroup "PBKDF2-HMAC-SHA256"
    [ testCase "basic round-trip" $ do
        let derived = unwrap $ pbkdf2 SHA256
              (BS8.pack "password") (BS8.pack "salt") (PBKDF2Params 1 32)
        -- Just verify it produces 32 bytes and doesn't crash
        secureBytesLength derived @?= 32
    , testCase "deterministic" $ do
        let d1 = fmap secureBytesToByteString $ pbkdf2 SHA256 (BS8.pack "pw") (BS8.pack "s") (PBKDF2Params 100 32)
            d2 = fmap secureBytesToByteString $ pbkdf2 SHA256 (BS8.pack "pw") (BS8.pack "s") (PBKDF2Params 100 32)
        d1 @?= d2
    , testCase "different passwords produce different output" $ do
        let d1 = fmap secureBytesToByteString $ pbkdf2 SHA256 "password1" "salt" (PBKDF2Params 100 32)
            d2 = fmap secureBytesToByteString $ pbkdf2 SHA256 "password2" "salt" (PBKDF2Params 100 32)
        assertBool "different passwords should differ" (d1 /= d2)
    , testCase "different salts produce different output" $ do
        let d1 = fmap secureBytesToByteString $ pbkdf2 SHA256 "password" "salt1" (PBKDF2Params 100 32)
            d2 = fmap secureBytesToByteString $ pbkdf2 SHA256 "password" "salt2" (PBKDF2Params 100 32)
        assertBool "different salts should differ" (d1 /= d2)
    , testCase "output length matches requested" $ do
        let d = unwrap $ pbkdf2 SHA256 "pw" "s" (PBKDF2Params 100 64)
