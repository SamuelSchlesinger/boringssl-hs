...
tests = testGroup "Scrypt"
  [ testCase "basic derivation (empty password/salt)" $ do
      ...
      let result = scrypt BS.empty BS.empty (ScryptParams 16 1 1 64)
      case result of
  ...
  , testCase "password and salt derivation" $ do
      ...
      let result = scrypt "password" "NaCl" (ScryptParams 1024 8 16 64)
      case result of
        Left err -> assertFailure ("scrypt returned Left: " ++ show err)
        Right derived ->
          secureBytesLength derived @?= 64
  ...
  , testCase "deterministic" $ do
      let d1 = fmap secureBytesToByteString $ scrypt "pw" "salt" (ScryptParams 16 1 1 32)
          d2 = fmap secureBytesToByteString $ scrypt "pw" "salt" (ScryptParams 16 1 1 32)
      d1 @?= d2

  , testCase "different passwords produce different output" $ do
      let d1 = fmap secureBytesToByteString $ scrypt "password1" "salt" (ScryptParams 16 1 1 32)
          d2 = fmap secureBytesToByteString $ scrypt "password2" "salt" (ScryptParams 16 1 1 32)
      assertBool "different passwords should differ" (d1 /= d2)

  , testCase "invalid N (not power of 2) returns Left" $ do
      let result = scrypt "pw" "salt" (ScryptParams 3 1 1 32)
      case result of
        Left _  -> return ()
        Right _ -> assertFailure "scrypt should reject invalid N"
  , testCase "different salts produce different output" $ do
      let d1 = fmap secureBytesToByteString $ scrypt "password" "salt1" (ScryptParams 16 1 1 32)
          d2 = fmap secureBytesToByteString $ scrypt "password" "salt2" (ScryptParams 16 1 1 32)
      assertBool "different salts should differ" (d1 /= d2)
  , testCase "output length matches requested" $ do
      case scrypt "pw" "s" (ScryptParams 16 1 1 64) of
  ...
  , testGroup "RFC 7914 known-answer vectors"
    [ testCase "vector 1: empty password/salt N=16 r=1 p=1" $ do
        ...
        case scrypt "" "" (ScryptParams 16 1 1 64) of
          Left err -> assertFailure ("scrypt failed: " ++ show err)
          Right derived ->
    ...
    , testCase "vector 2: password/NaCl N=1024 r=8 p=16" $ do
        ...
        case scrypt "password" "NaCl" (ScryptParams 1024 8 16 64) of
          Left err -> assertFailure ("scrypt failed: " ++ show err)
          Right derived ->
            secureBytesToByteString derived @?= expected
    , testCase "vector 3: pleaseletmein/SodiumChloride N=16384 r=8 p=1" $ do
        ...
        case scrypt "pleaseletmein" "SodiumChloride" (ScryptParams 16384 8 1 64) of
          Left err -> assertFailure ("scrypt failed: " ++ show err)
          Right derived ->
            secureBytesToByteString derived @?= expected
    ]
